...
1. **Initialisation:**
   ...
   - Set the proposal covariance $\Sigma_0 = \varepsilon_0 I_d$
...
With the lognormal model, the parameters of interest are $\phi = (\mu, \sigma, \theta)$. We generated data with the true parameter vector $\phi_{true} = (0, 1, 2)$ The statistics we chose are the median $med(x)$, the median absolute deviation defined as $mad(x) = med(|x_i - med(x)|)$, which is a robust estimator for the variance, and the $max(x)$.
